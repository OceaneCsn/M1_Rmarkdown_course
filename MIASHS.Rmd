...
+ Créez un nouveau document Rmarkdown via le menu File/New file/R Markdown (acceptez les éventuelles mises à jour). Laissez l'option HTML par défaut.

+ Personnalisez le template dnas l'entête YAML (auteur, titre...). Vous pouvez déjà le sauvegarder et les compiler avec le bouton knitr en haut de panel du script. Inspectez le résultat ainsi que le code l'ayant généré.


+ Le premier chunk de setup doit être conservé. Rajoutez-y l'import des packages que nous allons utiliser. Il doit donc contenir : 
...
Nous continuerons de le modifier plus tard dans l'énoncé. La suite du template peut être supprimée.

Au cours des étapes suivantes, utilisez régulièrement knitr pour compiler et visualiser le rendu du document.


1. **Donner le contexte et importer les données**
